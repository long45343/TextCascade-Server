...
原文的性能指标表（内存、广播 p95、冷启动等）在本轮修订中移除：项目从未建立度量设施（无 Benchmark 项目、无 p95 测量手段），保留未度量数字只会造成虚假承诺。如未来需要性能回归防护，应以独立规格与本仓库的测试设施一起重建。（协议层面保留的设计性质：广播单次 UTF-8 序列化、每连接有界发送队列、空闲路径只有心跳扫描。）
